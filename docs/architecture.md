...
|------------|------------|-----------|
| Research Agent | Gemini + Claude (Adversarial) | Generativo (No determinista) |
...
### 2.1 AI Research Agent (`AI_TREND_SCANNER_v3.1` + Adversarial Review)
...
- Validación adversarial Gemini ↔ Claude (2-3 iteraciones)

### 2.2 Flujo de Validación Adversarial

El sistema utiliza un patrón de **Reflexion** donde dos LLMs se auditan mutuamente:
...
│                                         │                    │
│  ┌──────────────────────────────────────▼───────────────┐   │
│  │ CONVERGENCIA: Solo sobreviven tendencias con         │   │
│  │ consenso técnico (sin objeciones pendientes)         │   │
│  └──────────────────────────────────────────────────────┘   │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
**Prompts utilizados:**
- `prompt_v3.1_gold.txt` — Generación inicial de tendencias y scoring
- `ADVERSARIAL_REVIEW.txt` — Auditoría de sesgos y validación cruzada
...
### 2.4 Optimization Engine (`engine.py`)
...
### 2.5 Visualization Layer (`app.py`)
...
sequenceDiagram
    ...
    Note over U,C: FASE 1: Validación Adversarial
    U->>G: Prompt AI Trend Scanner v3.1
    G->>G: Web Search (8 queries)
    G->>G: Bias Detection + Red Flags
    G-->>U: Output A (structured 12-col)
    
    U->>C: Output A + Adversarial Review Prompt
    C->>C: Audit biases, validate scoring
    C-->>U: Crítica B (objeciones + ajustes)
    
    U->>G: Output A + Crítica B + "Revisa y ajusta"
    G-->>U: Output A' (consensuado)
    
    Note over U,ST: FASE 2: Optimización
    U->>E: Paste Output A' to Excel
...
graph TB
    subgraph External["External Systems"]
        ...
        CLAUDE[("🤖 Claude<br/>LLM")]
    end
    
    subgraph SPO["Strategic Portfolio Optimizer"]
        subgraph Research["Research Layer (Adversarial)"]
            PROMPT["📄 AI Trend Scanner<br/>Prompt v3.1"]
            REVIEW["📄 Adversarial Review<br/>Prompt"]
        end
    ...
    PROMPT -->|Web Search| GEMINI
    GEMINI -->|8 queries| WEB
    GEMINI -->|Output A| REVIEW
    REVIEW -->|Audit| CLAUDE
    CLAUDE -->|Crítica B| GEMINI
    GEMINI -->|Output A' consensuado| EXCEL
...
### ADR-006: Validación Adversarial Multi-LLM

**Contexto:** Un solo LLM puede tener sesgos no detectados (vendor bias, hype bias).

**Decisión:** Implementar patrón de Reflexion con Gemini y Claude auditándose mutuamente.

**Razones:**
- Gemini y Claude tienen diferentes sesgos de entrenamiento
- La crítica cruzada reduce vendor bias (Google vs Anthropic)
- El consenso emerge solo cuando no hay objeciones pendientes

**Flujo:**
```
Gemini (Output A) → Claude (Crítica B) → Gemini (Output A' ajustado)
Repetir hasta convergencia (típicamente 2-3 iteraciones)
```

**Consecuencias:**
- ✅ Mayor robustez en detección de hype y vendor bias
- ✅ Scoring más defensible (auditado por dos modelos)
- ⚠️ Aumenta tiempo de ejecución (~3x vs single LLM)
- ⚠️ Requiere acceso a ambos modelos

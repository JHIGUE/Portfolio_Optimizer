...
with tabs[0]: # CONTEXTO (MEJORADO Y RESTAURADO)
    ...
    with col_nav:
        ...
        with st.expander("🎯 1. Plan Estratégico (Scatter & Ranking)"):
            st.write("Visualiza qué actividades entran en tu agenda (Verde) y cuáles se quedan fuera (Rojo) por falta de tiempo o valor. Es tu 'Sí' y tu 'No'.")
        with st.expander("📅 2. Gantt Secuencial"):
            st.write("Orden lógico de ejecución. No es cronológico simple: el algoritmo adelanta tareas pequeñas que desbloquean grandes hitos ('Score Heredado').")
        with st.expander("📈 3. Curva de Valor"):
            st.write("Análisis de sensibilidad. Te dice si estudiar 100 horas más aporta valor real o si estás llegando a un punto de saturación (rendimientos decrecientes).")
        with st.expander("🎲 4. Riesgo (Monte Carlo)"):
            st.write("Simulación de incertidumbre. Te dice cuánto tardarás *realmente* aplicando la Ley de Hofstadter (las cosas siempre tardan más de lo que esperas).")
...
with tabs[1]: # PLAN
    st.caption("📍 **Explicación:** Este gráfico posiciona cada actividad por **Esfuerzo (Horas)** vs **Impacto (Score)**. Las burbujas **VERDES** son las seleccionadas óptimamente.")
...
with tabs[2]: # GANTT
    st.caption("🗓️ **Explicación:** Cronograma inteligente. Prioriza las tareas 'desbloqueadoras' (Prerrequisitos) para maximizar el valor futuro disponible.")
...
with tabs[3]: # CURVA
    st.caption("📈 **Explicación:** ¿Merece la pena dedicar más horas? Si la curva se aplana, significa que añadir horas extra aporta poco valor estratégico.")
...
with tabs[4]: # AUDITORÍA
    st.caption("🔍 **Explicación:** Transparencia total. Revisa los datos brutos, cómo se calculó el Score y la Probabilidad Ajustada por tu perfil de Líder.")
...
with tabs[5]: # RIESGO
    st.caption("🎲 **Explicación:** Simulación de Monte Carlo (500 escenarios). Muestra la probabilidad real de terminar en tiempo considerando imprevistos.")
...
with tabs[6]: # COMPARADOR
    st.caption("🆚 **Explicación:** Compara tu escenario actual con otros guardados (ej. 'Plan Conservador' vs 'Plan Ambicioso') en la barra lateral.")
...
with tabs[7]: # EXPORTAR
    st.caption("📥 **Explicación:** Descarga el plan optimizado actual en Excel para compartirlo o trabajarlo offline.")
    if not df_opt.empty:
        buffer = io.BytesIO()
